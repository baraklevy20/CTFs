...
# Solution
We first try to `diff -a cattos.jpg kitters.jpg` to see differences but don't find anything useful, as it compares line by line.

We then try `cmp -lb cattos.jpg kitters.jpg` to compare the files byte-by-byte (`-l` will print the different bytes and `-b` will print them as characters) and we get:
```
  49734 160 p    231 M-^Y
  49735 151 i    235 M-^]
  49736 143 c    230 M-^X
...
1766742 175 }     13 ^K
```
We can see our flag! We can use `gawk` to print it nicely:
```
cmp -lb cattos.jpg kitters.jpg | gawk '{printf("%s", $3)}'
```
